...
describe('spawn working directory resolution', () => {
  describe('expandHome', () => {
    it('expands ~ to homedir', async () => {
      const { expandHome } = await import('../src/provider-launch.js')
  ...
  describe('resolveWorkingDir', () => {
    it('returns fallback when requested is undefined', async () => {
      const { resolveWorkingDir } = await import('../src/provider-launch.js')
  ...
  describe('launch command helpers', () => {
    it('adds Codex MCP env override as TOML inline table', async () => {
      const { codexMcpEnvOverride } = await import('../src/provider-launch.js')
    ...
    it('builds explicit Codex MCP command overrides', async () => {
      const { codexMcpOverrides } = await import('../src/provider-launch.js')
    ...
    it('uses PATH command directly for MCP launch fallback', async () => {
      const { reevesMcpServerLaunch } = await import('../src/provider-launch.js')
      expect(reevesMcpServerLaunch('reevesagents-orchestrator')).toEqual({ command: 'reevesagents-orchestrator', args: ['mcp'] })
    })

    it('builds explicit Claude MCP config with Reeves session env', async () => {
      const { claudeMcpConfig } = await import('../src/provider-launch.js')
    ...
    it('falls back to reevesagents path for test runner argv', async () => {
      const { resolveReevesMcpCommand } = await import('../src/provider-launch.js')
      expect(resolveReevesMcpCommand('/repo/node_modules/vitest/vitest.mjs')).toBe('reevesagents-orchestrator')
    })

    it('does not pass initial Codex task as a launch argument', async () => {
      const { launchCommandWithInitialTask } = await import('../src/provider-launch.js')
    ...
    it('execs Claude Code launch commands without prompt arguments', async () => {
      const { fullLaunchShellCommand } = await import('../src/provider-launch.js')
    ...
    it('builds a one-shot runner that gives the child non-tty stdio', async () => {
      const { oneShotRunnerSource } = await import('../src/provider-launch.js')
