...
import { buildCommand, detectAvailable, isProvider } from '../../../src/launcher/providers.js'
import {
  claudeMcpConfig,
  codexMcpOverrides,
...
type OrchestratorRunMode = 'spawner' | 'orchestrator'
...
function tmuxWindowName(mode: OrchestratorRunMode, role: AgentRole, provider: Provider, nickname: string): string {
...
function promptForMode(mode: OrchestratorRunMode, role: AgentRole, task: string): string {
...
export function startRun(request: StartRunRequest, options: RuntimeOptions = {}): { run: RunRecord, agents: AgentRecord[] } {
  ...
  const mode = request.mode ?? 'spawner'
...
export function spawnWorker(request: SpawnWorkerRequest, options: RuntimeOptions = {}): AgentRecord {
  ...
  const mode: OrchestratorRunMode = run.mode === 'spawner' ? 'spawner' : 'orchestrator'
