...
run не падал из-за отсутствующего CLI; окно, застрявшее на экране входа,
поймает `peek` (ниже). `reevesagents doctor --json` возвращает то же самое в
машиночитаемом JSON.
...
провайдерский CLI есть на PATH, и перечисляет отсутствующие, поэтому опечатка
...
существующий run вместо создания нового), `--json` (напечатать id run'а и
агентов в JSON вместо текста).
...
закрывает и очистку: `delete <agent-id>` и `delete-run <run-id>` удаляют записи
о завершённых агентах и run'ах, а `delete-history <id>` удаляет запись из
архива.
...
- Не запускайте провайдера, который не установлен или без выполненного входа;
  в первом случае spawn откажет сразу, во втором окно так и останется стоять на
  экране входа и работу не начнёт.
- Не запускайте `stop`, `kill` и команды `delete` без `--yes`; это и есть
  деструктивные команды.
- Не цельтесь в нативный Windows; работайте внутри WSL с установленными там
  tmux и CLI.
