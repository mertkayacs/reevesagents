...
Чтобы наращивать рабочее пространство по одному агенту за раз,
`reevesagents add <spec...>` добавляет агентов в самый недавний активный
run без id run'а (передайте `--run <run-id>`, чтобы указать другой).
Принимает те же флаги для каждого агента, что и `spawn`. Добавленные так
агенты работают бок о бок и не управляют друг другом: такое происходит,
только когда подключён Agent control MCP.
