...
Connected agent coordination is PRE-BETA test code under `packages/orchestrator`. It must not leak into the main package install path, TUI flow, CLI help, or packed root tarball. Root docs may mention it only to clarify that the main app is the spawner and orchestration is not stable.
